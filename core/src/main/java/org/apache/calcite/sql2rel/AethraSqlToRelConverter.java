 ...
 * This class once more is an AethraDB specific version of {@link SqlToRelConverter} which aims
 * to reduce uncesssary overhead as much as possible.
 */
@SuppressWarnings("UnstableApiUsage")
@Value.Enclosing
public class AethraSqlToRelConverter {
  ...
  private AethraSubQueryConverter subQueryConverter;
  ...
  //~ Constructors -----------------------------------------------------------
  /* Creates a converter. */
  public AethraSqlToRelConverter(
    ...
    this.subQueryConverter = new AethraNoOpSubQueryConverter();
   ...
   */
  public void setSubQueryConverter(AethraSubQueryConverter converter) {
   ...
   *
   * @param query           The validated query to convert
   * @param top             Whether the query is top-level, say if its result
   *                        will become a JDBC result set; <code>false</code> if
   *                        the query will be part of a view.
   */
  public RelRoot convertQuery(SqlNode query, final boolean top) {
    RelNode result = convertQueryRecursive(query, top, null).rel;

// Functionality does not seem used in AethraDB
//    if (top) {
//      if (isStream(query)) {
...
//    }
  ...
  protected void convertPivot(Blackboard bb, SqlPivot pivot) {
    ...
    // Gather fields.
    final AethraAggConverter aggConverter = AethraAggConverter.create(pivotBb);
  ...
  protected void convertCollectionTable(
    ...
    if (operator == SqlStdOperatorTable.TABLESAMPLE) {
      ...
      RelNode converted = convertQuery(query, false).rel;
  ...
  protected void afterTableFunction(
      AethraSqlToRelConverter.Blackboard bb,
      SqlCall call,
      LogicalTableFunctionScan callRel) {
  }
  ...
  protected void convertAgg(Blackboard bb, SqlSelect select,
    ...
    final AethraAggConverter aggConverter =
        AethraAggConverter.create(bb, (AggregatingSelectScope) validator().getSelectScope(select));
  ...
  private RelOptTable.ToRelContext createToRelContext(List<RelHint> hints) {
    // Views are not used in AethraDB, so ViewExpander is not required.
    return ViewExpanders.toRelContext(null, cluster, hints);
  }
  ...
  private RelNode convertMultisets(final List<SqlNode> operands,
    ...
    for (int i = 0; i < operands.size(); i++) {
      ...
      case ARRAY_QUERY_CONSTRUCTOR:
      case MAP_QUERY_CONSTRUCTOR:
        final RelRoot root = convertQuery(call.operand(0), true);
        input = root.rel;
        break;
      default:
        lastList.add(operand);
  ...
  public RelRoot convertWith(SqlWith with, boolean top) {
    return convertQuery(with.body, top);
  }
  ...
  protected class Blackboard implements SqlRexContext, SqlVisitor<RexNode>,
     ...
     */
    @Nullable AethraAggConverter agg;
    ...
    @Override public RexNode convertExpression(SqlNode expr) {
      ...
      AethraAggConverter agg = this.agg;
      if (agg != null) {
   ...
   */
  private static class AethraNoOpSubQueryConverter implements AethraSubQueryConverter {
    ...
    @Override public RexNode convertSubQuery(
        SqlCall subQuery,
        AethraSqlToRelConverter parentConverter,
        boolean isExists,
        boolean isExplain) {
  ...
  /** Returns a default {@link SqlToRelConverter.Config}. */
